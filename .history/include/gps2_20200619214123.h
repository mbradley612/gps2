...
/* create the gps2 device and set the event handler. The handler callback will be called when the GPS is initialized, when a GPS fix
  is acquired or lost and whenever there is a location update */

struct gps2 *gps2_create_uart(uint8_t uart_no, struct mgos_uart_config, gps2_ev_handler handler, void *handler_user_data);
