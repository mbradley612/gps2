...
void gps2_get_position(struct gps2 *dev, float *lat, float *lon, unsigned long *fix_age) {
...
void process_gga_frame(struct gps2 *gps_dev, struct minmea_sentence_gga gga_frame) {
  ...
  LOG(LL_DEBUG,("Processing RMC frame"));
  gps_dev->position.lat = gga_frame.latitude.value / gga_frame.latitude.scale;;
  gps_dev->position.lon = gga_frame.longitude.value / gga_frame.longitude.scale;;
  gps_dev->position.timestamp = mgos_uptime() / 1000;
