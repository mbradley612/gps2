...
void gps2_get_position(struct gps2 *dev, unsigned float *lat, unsigned long *lon, unsigned long *fix_age) {
  *lat = dev->position.lat;
  *lon = dev->position.lon;
  *fix_age = (mgos_uptime() / 1000) - dev->position.timestamp;
}
...
void gps2_get_satellites(struct gps2 *dev, int *satellites_tracked, unsigned long *age) {
  ...
  *age = (mgos_uptime() / 1000) - dev->satellites.timestamp;
...
void gps2_get_fix_quality(struct gps2 *dev, int *fix_quality, unsigned long *age) {
  ...
  *age = (mgos_uptime() /1000) - dev->fix_quality.timestamp;
...
void process_rmc_frame(struct gps2 *gps_dev, struct minmea_sentence_rmc rmc_frame) {
  ...
  gps_dev->position.lat = rmc_frame.latitude.value / rmc_frame.latitude.scale;
  gps_dev->position.lon = rmc_frame.longitude.value / rmc_frame.longitude.scale;
  gps_dev->position.timestamp = mgos_uptime() / 1000;
  /* speed */
  gps_dev->speed.speed = rmc_frame.speed.value;
  gps_dev->speed.timestamp = mgos_uptime() / 1000;
  /* course */
  gps_dev->course.course = rmc_frame.course.value;
  gps_dev->course.timestamp = mgos_uptime() /1000;
  /* date time */
  ...
  gps_dev->datetime.timestamp = mgos_uptime() / 1000;
...
void process_gga_frame(struct gps2 *gps_dev, struct minmea_sentence_gga gga_frame) {
  ...
  LOG(LL_DEBUG,("Processing RMC frame"));
  gps_dev->position.lat = gga_frame.latitude.value / rmc_frame.latitude.scale;;
  gps_dev->position.lon = gga_frame.longitude.value / rmc_frame.longitude.scale;;
  gps_dev->position.timestamp = mgos_uptime() / 1000;

   /* satellites tracked */
  gps_dev->satellites.satellites_tracked = gga_frame.satellites_tracked;
  gps_dev->satellites.timestamp = mgos_uptime() /1000;
