...
struct gps2 {
  ...
  struct gps_fix_quality_reading fix_quality_reading;
...
void gps2_get_datetime(struct gps2 *dev, struct gps2_datetime *datetime, int64_t *age ) {
...
void gps2_get_fix_quality(struct gps2 *dev, int *fix_quality, int64_t *age) {
  *fix_quality = dev->fix_quality_reading.fix_quality;
  *age = mgos_uptime_micros()  - dev->fix_quality_reading.timestamp;
...
void process_rmc_frame(struct gps2 *gps_dev, struct minmea_sentence_rmc rmc_frame) {
  ...
  /* lon and lat */
  gps_dev->location_reading.location.lat = minmea_tocoord(&rmc_frame.latitude);
  gps_dev->location_reading.location.lon = minmea_tocoord(&rmc_frame.longitude);
...
/* location including speed and course and age of fix in milliseconds 
   this is derived from the most recent RMC sentence*/
void gps2_global_get_location(struct gps2_location *location, int64_t *fix_age) {
  gps2_get_location(gps2_get_global_device(), location, fix_age);
}
 
/* date and time */
void gps2_global_get_datetime(struct gps2_datetime *datetime, int64_t *age ) {
  gps2_get_datetime(gps2_get_global_device(), datetime, age);
}

/* unix time now in milliseconds and microseconds adjusted for age*/
void gps2_global_get_unixtime(time_t *unix_time, int64_t *microseconds) {
  gps2_get_unixtime(gps2_get_global_device(),unix_time,microseconds);
}

/* satellites used in last full GPGGA sentence */
void gps2_global_get_satellites( int *satellites_tracked, int64_t *age) {
